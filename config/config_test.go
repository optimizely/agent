...
func TestDefaultConfig(t *testing.T) {
	...
	assert.Equal(t, 0, conf.Client.MaxConns)
	assert.Equal(t, "8080", conf.Client.Port)
	assert.Equal(t, make([]OAuthClientCredentials, 0), conf.Client.Auth.Clients)
	assert.Equal(t, "", conf.Client.Auth.HMACSecret)
	assert.Equal(t, time.Duration(0), conf.Client.Auth.TTL)
