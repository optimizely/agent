...
func WithAPIRouter(opt *APIOptions, r chi.Router) {
	...
	r.Route("/v1", func(r chi.Router) {
		...
		r.With(activateTimer, opt.oAuthMiddleware, contentTypeMiddleware, middleware.AddTracing("activateHandler", "Activate")).Post("/activate", opt.activateHandler)
		r.With(decideTimer, opt.oAuthMiddleware, contentTypeMiddleware, middleware.AddTracing("decideHandler", "Decide")).Post("/decide", opt.decideHandler)
		...
		r.With(overrideTimer, opt.oAuthMiddleware, contentTypeMiddleware, middleware.AddTracing("overrideHandler", "Override")).Post("/override", opt.overrideHandler)
		r.With(lookupTimer, opt.oAuthMiddleware, contentTypeMiddleware, middleware.AddTracing("lookupHandler", "Lookup")).Post("/lookup", opt.lookupHandler)
		r.With(saveTimer, opt.oAuthMiddleware, contentTypeMiddleware, middleware.AddTracing("saveHandler", "Save")).Post("/save", opt.saveHandler)
		r.With(sendOdpEventTimer, opt.oAuthMiddleware, contentTypeMiddleware, middleware.AddTracing("sendOdpEventHandler", "SendOdpEvent")).Post("/send-odp-event", opt.sendOdpEventHandler)
		r.With(opt.oAuthMiddleware, middleware.AddTracing("notificationHandler", "SendNotificationEvent")).Get("/notifications/event-stream", opt.nStreamHandler)
	})

	r.With(createAccesstokenTimer, middleware.AddTracing("authHandler", "AuthToken")).Post("/oauth/token", opt.oAuthHandler)
